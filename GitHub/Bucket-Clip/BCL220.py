    ...
    for pr2 in range(len(xi_w_list)):
        ...
        for pr3 in range(len(BG_list)):
            ...
            for prb in range(4):
                ...
                for epoch in range(iteration):
                    ...
                    for k in range(B):
                        if HW_sign[k]==1:#censoring
                            last_fc1_w_g_B[k] = (float(np.random.randn(1))) * net[k].fc1.weight.grad.clone()
                            last_fc2_w_g_B[k] = (float(np.random.randn(1))) * net[k].fc2.weight.grad.clone()
                            last_fc3_w_g_B[k] = (float(np.random.randn(1))) * net[k].fc3.weight.grad.clone()
                            last_conv1_w_g_B[k] = (float(np.random.randn(1))) * net[k].conv1.weight.grad.clone()
                            last_conv2_w_g_B[k] = (float(np.random.randn(1))) * net[k].conv2.weight.grad.clone()
                            last_fc1_b_g_B[k] = (float(np.random.randn(1))) * net[k].fc1.bias.grad.clone()
                            last_fc2_b_g_B[k] = (float(np.random.randn(1))) * net[k].fc2.bias.grad.clone()
                            last_fc3_b_g_B[k] = (float(np.random.randn(1))) * net[k].fc3.bias.grad.clone()
                            last_conv1_b_g_B[k] = (float(np.random.randn(1))) * net[k].conv1.bias.grad.clone()
                            last_conv2_b_g_B[k] = (float(np.random.randn(1))) * net[k].conv2.bias.grad.clone()
                            temp = []
                ...
                pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_bcl220%d.csv'%pr_c)
                pd.DataFrame(HS_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HSsum_bcl220%d.csv'%pr_c)
                pd.DataFrame(HW_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HWsum_bcl220%d.csv'%pr_c)
                pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_bcl220%d.csv'%pr_c)
                pr_c += 1
