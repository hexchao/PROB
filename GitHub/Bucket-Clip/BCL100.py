...
def clipping(tau, It, para_old, paras):
    v = para_old
    u = paras
    tempC = [[] for k in range(BGroup)]
    for i in range(It):
        for ii in range(BGroup):
            tempC[ii] = (torch.sub(u[ii],v) * min(1,tau/torch.norm(torch.sub(u[ii],v))))[None]
        v += torch.mean(torch.cat(tempC, dim=0), dim=0)
    return v
...
def censor_W(k):
    paranorm_list = []
    for pl in range(10):
        paranorm_list.append(torch.norm(last_fc1_w_g[k]-net[k].fc1.weight.grad.clone()))
        paranorm_list.append(torch.norm(last_fc2_w_g[k]-net[k].fc2.weight.grad.clone()))
        paranorm_list.append(torch.norm(last_fc3_w_g[k]-net[k].fc3.weight.grad.clone()))
        paranorm_list.append(torch.norm(last_conv1_w_g[k]-net[k].conv1.weight.grad.clone()))
        paranorm_list.append(torch.norm(last_conv2_w_g[k]-net[k].conv2.weight.grad.clone()))
        paranorm_list.append(torch.norm(last_fc1_b_g[k]-net[k].fc1.bias.grad.clone()))
        paranorm_list.append(torch.norm(last_fc2_b_g[k]-net[k].fc2.bias.grad.clone()))
        paranorm_list.append(torch.norm(last_fc3_b_g[k]-net[k].fc3.bias.grad.clone()))
        paranorm_list.append(torch.norm(last_conv1_b_g[k]-net[k].conv1.bias.grad.clone()))
        paranorm_list.append(torch.norm(last_conv2_b_g[k]-net[k].conv2.bias.grad.clone()))
    return sum(paranorm_list)
...
for pr1 in range(len(xi_s_list)):
    ...
    for pr2 in range(len(xi_w_list)):
        ...
        for pr3 in range(len(BG_list)):
            ...
            last_fc1_w_g = [[] for k in range(Node)]
            last_fc2_w_g = [[] for k in range(Node)]
            last_fc3_w_g = [[] for k in range(Node)]
            last_conv1_w_g = [[] for k in range(Node)]
            last_conv2_w_g = [[] for k in range(Node)]
            last_fc1_b_g = [[] for k in range(Node)]
            last_fc2_b_g = [[] for k in range(Node)]
            last_fc3_b_g = [[] for k in range(Node)]
            last_conv1_b_g = [[] for k in range(Node)]
            last_conv2_b_g = [[] for k in range(Node)]
            last0_fc1_w_g = []
            last0_fc2_w_g = []
            last0_fc3_w_g = []
            last0_conv1_w_g = []
            ...
            for epoch in range(iteration):
                ...
                for k in range(Node):
                    if HW_sign[k]==1:#censoring子节点发送的
                        last_fc1_w_g[k] = net[k].fc1.weight.grad.clone()
                        last_fc2_w_g[k] = net[k].fc2.weight.grad.clone()
                        last_fc3_w_g[k] = net[k].fc3.weight.grad.clone()
                        last_conv1_w_g[k] = net[k].conv1.weight.grad.clone()
                        last_conv2_w_g[k] = net[k].conv2.weight.grad.clone()
                        last_fc1_b_g[k] = net[k].fc1.bias.grad.clone()
                        last_fc2_b_g[k] = net[k].fc2.bias.grad.clone()
                        last_fc3_b_g[k] = net[k].fc3.bias.grad.clone()
                        last_conv1_b_g[k] = net[k].conv1.bias.grad.clone()
                        last_conv2_b_g[k] = net[k].conv2.bias.grad.clone()
                        temp = []
                        temp.append(net[k].fc1.weight.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].fc2.weight.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].fc3.weight.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].conv1.weight.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].conv2.weight.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].fc1.bias.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].fc2.bias.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].fc3.bias.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].conv1.bias.grad.clone().view([-1]).unsqueeze(0).T)
                        temp.append(net[k].conv2.bias.grad.clone().view([-1]).unsqueeze(0).T)
                ...
                CLine = clipping(tau, It, para_0, BucketGroup)

                CMark = 0
                net0.fc1.weight.grad = CLine[CMark:CMark+fc1_w_len].T.view(fc1_w_size)
                CMark += fc1_w_len
                net0.fc2.weight.grad = CLine[CMark:CMark+fc2_w_len].T.view(fc2_w_size)
                CMark += fc2_w_len
                net0.fc3.weight.grad = CLine[CMark:CMark+fc3_w_len].T.view(fc3_w_size)
                CMark += fc3_w_len
                net0.conv1.weight.grad = CLine[CMark:CMark+conv1_w_len].T.view(conv1_w_size)
                CMark += conv1_w_len
                net0.conv2.weight.grad = CLine[CMark:CMark+conv2_w_len].T.view(conv2_w_size)
                CMark += conv2_w_len
                net0.fc1.bias.grad = CLine[CMark:CMark+fc1_b_len].T.view(fc1_b_size)
                CMark += fc1_b_len
                net0.fc2.bias.grad = CLine[CMark:CMark+fc2_b_len].T.view(fc2_b_size)
                CMark += fc2_b_len
                net0.fc3.bias.grad = CLine[CMark:CMark+fc3_b_len].T.view(fc3_b_size)
                CMark += fc3_b_len
                net0.conv1.bias.grad = CLine[CMark:CMark+conv1_b_len].T.view(conv1_b_size)
                CMark += conv1_b_len
                net0.conv2.bias.grad = CLine[CMark:CMark+conv2_b_len].T.view(conv2_b_size)
                ...
                HW_sum += sum(HW_sign)
                if epoch%count_iter==0:
                    print(epoch, float(censor_W(0)), float(censor_W(9)))
                for k in range(Node):
                    optimizer[k].step()
            ...
            pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_bcl100%d.csv'%pr_c)
            pd.DataFrame(HS_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HSsum_bcl100%d.csv'%pr_c)
            pd.DataFrame(HW_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HWsum_bcl100%d.csv'%pr_c)
            pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_bcl100%d.csv'%pr_c)
            pr_c += 1
