...
pr_c = 10
stepsize = 0.002
alpha = 0.002 # 0.0015 for momentum
penal = 0.001
regu = 0.01
beta = 0.2
L_Huber = 5 * pow(0.1, 3)
...
for pr_b in Blist:
    ...
    for epoch in range(iteration):
        ...
        if xi == 0:
            ...
            # for pi, data in enumerate(msg):
            #     tmp = data[B:].sum(dim=0)
            #     data[0:B].data.copy_(-tmp.data.clone() / B)
            for k in range(B, Node):
            ...
            for k in range(Node):
                for (para0, para) in zip(net0.parameters(), net[k].parameters()):
                    para.grad = beta / p * (para.data - para0.data).detach()
    ...
    pr_c = 10 + pr_b
    pd.DataFrame(accuracy_list).to_csv('./result/Acc_BREPN241{}_P{}_M{}_N{}_0.004.csv'.format(pr_c, 1 / p, mmm, Node))
