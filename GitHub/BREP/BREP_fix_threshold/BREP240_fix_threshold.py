...
pr_c = 10
stepsize = 0.002
alpha = 0.002 #0.0015 for p1/50 mmm
penal = 0.001
regu = 0.01
beta = 0.2
L_Huber = 5e-3
...
for pr_b in Blist:
    ...
    for epoch in range(iteration):
        ...
        if xi == 0:
            ...
            # buf = []
            # d = 0.
            # for i, data in enumerate(msg[-1]):
            #     buf.append((-1 / B) * torch.stack([msg[k][i] for k in range(B, Node)]).sum(dim=0))
            #     d = max(d, torch.norm(buf[-1], p=float('inf')).item())
            # for k in range(B):
            #     diff[k] = d

            for k in range(B, Node):
                for pi, para in enumerate(net[k].parameters()):
                    msg[pi][k].data.copy_(para.data)
            for pi, (data, para0) in enumerate(zip(msg, net0.parameters())):
                tmp = data[B:].sum(dim=0)
                data[0:B].data.copy_(-tmp.data / B)
                data.data.sub_(para0.data)
                # max_msg_h = max(max_msg_h, data[B:].max().item())
                # max_msg_b = max(max_msg_b, data[0:B].max().item())
            ...
            # for pi, para in enumerate(net0.parameters()):
            #     inc = torch.stack([msg[k][pi] for k in range(B, Node)]).sum(dim=0)
            #     para.data.sub_(inc.data, alpha=alpha/p)
            #     para.data.sub_(buf[pi].data, alpha=alpha/p*B)
    ...
    pr_c = 10 + pr_b
    pd.DataFrame(accuracy_list).to_csv('./result/Acc_BREPFIX{}_N241{}_P{}_M{}_N{}_0.004.csv'.format(L_Huber, pr_c, 1 / p, mmm, Node))
