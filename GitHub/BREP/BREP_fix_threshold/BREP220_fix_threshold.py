...
for pr_b in Blist:
    ...
    for epoch in range(iteration):
        ...
        if xi == 0:
            ...
            #     d = 0.
            #     for (para, para0) in zip(net[k].parameters(), net0.parameters()):
            #         msg[k].append((float(np.random.randn(1))) + para.data.clone() - para0.data.clone())
            #         d = max(d, torch.norm(msg[k][-1], p=float('inf')).item())
            #     diff[k] = d
            for k in range(B, Node):
            ...
            for k in range(B):
                for pi, (para, para0) in enumerate(zip(net[k].parameters(), net0.parameters())):
                    msg[pi][k].data.copy_((float(np.random.randn(1))) + (para.data - para0.data))
    ...
    pr_c = 10 + pr_b
    pd.DataFrame(accuracy_list).to_csv('./result/Acc_BREPFIX{}_N221{}_P{}_M{}_N{}.csv'.format(L_Huber, pr_c, 1/p, mmm, Node))
