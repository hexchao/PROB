...
def DistinguishHuber(msg, lh):
    ind_small = torch.where(msg.abs() <= lh)
    ind_large = torch.where(msg.abs() > lh)
...
def DistinguishHuber1(msg, lh):
    for data in msg:
        data_abs = data.abs()
        ind_small = torch.where(data_abs - lh <= 0)
        ind_large = torch.where(data_abs - lh > 0)
        data[ind_large] = (-1) * beta * lh * (data[ind_large].sign())
...
for pr_b in Blist:
    ...
    for epoch in range(iteration):
        ...
        if xi == 0:
            ...
            #     d = 0.
            #     for (para, para0) in zip(net[k].parameters(), net0.parameters()):
            #         msg[k].append(para0.data.clone() - para.data.clone())
            #         d = max(d, torch.norm(msg[k][-1], p=float('inf')).item())
            #     diff[k] = d
            for k in range(B, Node):
                for pi, (para, para0) in enumerate(zip(net[k].parameters(), net0.parameters())):
                    msg[pi][k].data.copy_(para.data - para0.data)
            for k in range(B):
                for pi, (para, para0) in enumerate(zip(net[k].parameters(), net0.parameters())):
                    msg[pi][k].data.copy_(-para0.data - para.data)
            DistinguishHuber1(msg, L_Huber)
            for k in range(Node):
            ...
            #
            # for pi, para in enumerate(net0.parameters()):
            #     inc = torch.stack([msg[k][pi] for k in range(Node)]).sum(dim=0)
            #     para.data.sub_(inc.data, alpha=alpha/p)
        ...
        #     netk_norm = ParaNorm1(net[k])
        #     if netk_norm > 1000:
        #         net[k].fc1.weight.data = 1000 / netk_norm * net[k].fc1.weight.data.clone()
        #         net[k].fc2.weight.data = 1000 / netk_norm * net[k].fc2.weight.data.clone()
        #         net[k].fc3.weight.data = 1000 / netk_norm * net[k].fc3.weight.data.clone()
        #         net[k].conv1.weight.data = 1000 / netk_norm * net[k].conv1.weight.data.clone()
        #         net[k].conv2.weight.data = 1000 / netk_norm * net[k].conv2.weight.data.clone()
        #         net[k].fc1.bias.data = 1000 / netk_norm * net[k].fc1.bias.data.clone()
        #         net[k].fc2.bias.data = 1000 / netk_norm * net[k].fc2.bias.data.clone()
        #         net[k].fc3.bias.data = 1000 / netk_norm * net[k].fc3.bias.data.clone()
        #         net[k].conv1.bias.data = 1000 / netk_norm * net[k].conv1.bias.data.clone()
        #         net[k].conv2.bias.data = 1000 / netk_norm * net[k].conv2.bias.data.clone()
        #
        # net0_norm = ParaNorm1(net0)
        # if net0_norm > 1000:
        #     net0.fc1.weight.data = 1000 / net0_norm * net0.fc1.weight.data.clone()
        #     net0.fc2.weight.data = 1000 / net0_norm * net0.fc2.weight.data.clone()
        #     net0.fc3.weight.data = 1000 / net0_norm * net0.fc3.weight.data.clone()
        #     net0.conv1.weight.data = 1000 / net0_norm * net0.conv1.weight.data.clone()
        #     net0.conv2.weight.data = 1000 / net0_norm * net0.conv2.weight.data.clone()
        #     net0.fc1.bias.data = 1000 / net0_norm * net0.fc1.bias.data.clone()
        #     net0.fc2.bias.data = 1000 / net0_norm * net0.fc2.bias.data.clone()
        #     net0.fc3.bias.data = 1000 / net0_norm * net0.fc3.bias.data.clone()
        #     net0.conv1.bias.data = 1000 / net0_norm * net0.conv1.bias.data.clone()
        #     net0.conv2.bias.data = 1000 / net0_norm * net0.conv2.bias.data.clone()
    ...
    pr_c = 10 + pr_b
    pd.DataFrame(accuracy_list).to_csv('./result/Acc_BREPFIX{}_N211{}_P{}_M{}_N{}.csv'.format(L_Huber, pr_c, 1 / p, mmm, Node))
