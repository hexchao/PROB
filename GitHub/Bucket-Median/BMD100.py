...
def median_aggregation(paras):
    return torch.cat([paras[k].T for k in range(len(paras))], dim=0).median(dim=0).values
    ...
    for pr2 in range(len(xi_w_list)):
        ...
        for pr3 in range(len(BG_list)):
            ...
            for epoch in range(iteration):
                ...
                MLine = median_aggregation(BucketGroup)

                CMark = 0
                net0.fc1.weight.grad = MLine[CMark:CMark+fc1_w_len].T.view(fc1_w_size)
                CMark += fc1_w_len
                net0.fc2.weight.grad = MLine[CMark:CMark+fc2_w_len].T.view(fc2_w_size)
                CMark += fc2_w_len
                net0.fc3.weight.grad = MLine[CMark:CMark+fc3_w_len].T.view(fc3_w_size)
                CMark += fc3_w_len
                net0.conv1.weight.grad = MLine[CMark:CMark+conv1_w_len].T.view(conv1_w_size)
                CMark += conv1_w_len
                net0.conv2.weight.grad = MLine[CMark:CMark+conv2_w_len].T.view(conv2_w_size)
                CMark += conv2_w_len
                net0.fc1.bias.grad = MLine[CMark:CMark+fc1_b_len].T.view(fc1_b_size)
                CMark += fc1_b_len
                net0.fc2.bias.grad = MLine[CMark:CMark+fc2_b_len].T.view(fc2_b_size)
                CMark += fc2_b_len
                net0.fc3.bias.grad = MLine[CMark:CMark+fc3_b_len].T.view(fc3_b_size)
                CMark += fc3_b_len
                net0.conv1.bias.grad = MLine[CMark:CMark+conv1_b_len].T.view(conv1_b_size)
                CMark += conv1_b_len
                net0.conv2.bias.grad = MLine[CMark:CMark+conv2_b_len].T.view(conv2_b_size)
            ...
            pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_BMD100%d.csv'%pr_c)
            pd.DataFrame(HS_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HSsum_BMD100%d.csv'%pr_c)
            pd.DataFrame(HW_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HWsum_BMD100%d.csv'%pr_c)
            pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_BMD100%d.csv'%pr_c)
            pr_c += 1
