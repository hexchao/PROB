...
def SignComp(k):
    sign_comp = 0
    sign_comp += len ((sign_fc1_w_0_N[k] - sign_fc1_w_old[k]).view([-1]).nonzero())
    sign_comp += len ((sign_fc2_w_0_N[k] - sign_fc2_w_old[k]).view([-1]).nonzero())
    sign_comp += len ((sign_fc3_w_0_N[k] - sign_fc3_w_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_conv1_w_0_N[k] - sign_conv1_w_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_conv2_w_0_N[k] - sign_conv2_w_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_fc1_b_0_N[k] - sign_fc1_b_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_fc2_b_0_N[k] - sign_fc2_b_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_fc3_b_0_N[k] - sign_fc3_b_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_conv1_b_0_N[k] - sign_conv1_b_old[k]).view([-1]).nonzero())
    sign_comp += len((sign_conv2_b_0_N[k] - sign_conv2_b_old[k]).view([-1]).nonzero())
    return sign_comp
...
for pr1 in range(len(xi_s_list)):
    ...
    sign_fc1_w_old =[[] for k in range(Node)]
    sign_fc2_w_old =[[] for k in range(Node)]
    sign_fc3_w_old =[[] for k in range(Node)]
    sign_conv1_w_old =[[] for k in range(Node)]
    sign_conv2_w_old =[[] for k in range(Node)]
    sign_fc1_b_old =[[] for k in range(Node)]
    sign_fc2_b_old =[[] for k in range(Node)]
    sign_fc3_b_old =[[] for k in range(Node)]
    sign_conv1_b_old =[[] for k in range(Node)]
    sign_conv2_b_old =[[] for k in range(Node)]
    ...
    for epoch in range(iteration):
        ...
        for k in range(Node):
            sign_fc1_w_old[k] = sign_fc1_w_0_N[k].clone()
            sign_fc2_w_old[k] = sign_fc2_w_0_N[k].clone()
            sign_fc3_w_old[k] = sign_fc3_w_0_N[k].clone()
            sign_conv1_w_old[k] = sign_conv1_w_0_N[k].clone()
            sign_conv2_w_old[k] = sign_conv2_w_0_N[k].clone()
            sign_fc1_b_old[k] = sign_fc1_b_0_N[k].clone()
            sign_fc2_b_old[k] = sign_fc2_b_0_N[k].clone()
            sign_fc3_b_old[k] = sign_fc3_b_0_N[k].clone()
            sign_conv1_b_old[k] = sign_conv1_b_0_N[k].clone()
            sign_conv2_b_old[k] = sign_conv2_b_0_N[k].clone()
        for k in range(Node):
        ...
        net0.fc1.weight.grad = regu * net0.fc1.weight.data.clone() + torch.stack([(-1) * sign_fc1_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.fc2.weight.grad = regu * net0.fc2.weight.data.clone() + torch.stack([(-1) * sign_fc2_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.fc3.weight.grad = regu * net0.fc3.weight.data.clone() + torch.stack([(-1) * sign_fc3_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.conv1.weight.grad = regu * net0.conv1.weight.data.clone() + torch.stack([(-1) * sign_conv1_w_0_N[k] for k in range(10)],4).sum(axis=4)
        net0.conv2.weight.grad = regu * net0.conv2.weight.data.clone() + torch.stack([(-1) * sign_conv2_w_0_N[k] for k in range(10)],4).sum(axis=4)
        net0.fc1.bias.grad = regu * net0.fc1.bias.data.clone() + torch.stack([(-1) * sign_fc1_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.fc2.bias.grad = regu * net0.fc2.bias.data.clone() + torch.stack([(-1) * sign_fc2_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.fc3.bias.grad = regu * net0.fc3.bias.data.clone() + torch.stack([(-1) * sign_fc3_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.conv1.bias.grad = regu * net0.conv1.bias.data.clone() + torch.stack([(-1) * sign_conv1_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.conv2.bias.grad = regu * net0.conv2.bias.data.clone() + torch.stack([(-1) * sign_conv2_b_0_N[k] for k in range(10)],1).sum(axis=1)
        optimizer0.step()
    ...
    pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_LL100%d.csv'%pr_c)
    pd.DataFrame(HS_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HSsum_LL100%d.csv'%pr_c)
    pd.DataFrame(HW_sum_list).to_csv('/home/hexchao/Simulation_CIFAR10/HWsum_LL100%d.csv'%pr_c)
    pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_LL100%d.csv'%pr_c)
    pd.DataFrame(dist).to_csv('/home/hexchao/Simulation_CIFAR10/Dist_LL100%d.csv'%pr_c)
    pr_c += 1
