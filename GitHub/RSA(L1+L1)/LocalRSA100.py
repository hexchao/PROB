...
for pr_lc in range(len(lc_list)):
    ...
    for epoch in range(iteration):
        ...
        net0.fc1.weight.grad = regu * net0.fc1.weight.data.clone() + penal * torch.stack([(-1) * sign_fc1_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.fc2.weight.grad = regu * net0.fc2.weight.data.clone() + penal * torch.stack([(-1) * sign_fc2_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.fc3.weight.grad = regu * net0.fc3.weight.data.clone() + penal * torch.stack([(-1) * sign_fc3_w_0_N[k] for k in range(10)],2).sum(axis=2)
        net0.conv1.weight.grad = regu * net0.conv1.weight.data.clone() + penal * torch.stack([(-1) * sign_conv1_w_0_N[k] for k in range(10)],4).sum(axis=4)
        net0.conv2.weight.grad = regu * net0.conv2.weight.data.clone() + penal * torch.stack([(-1) * sign_conv2_w_0_N[k] for k in range(10)],4).sum(axis=4)
        net0.fc1.bias.grad = regu * net0.fc1.bias.data.clone() + penal * torch.stack([(-1) * sign_fc1_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.fc2.bias.grad = regu * net0.fc2.bias.data.clone() + penal * torch.stack([(-1) * sign_fc2_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.fc3.bias.grad = regu * net0.fc3.bias.data.clone() + penal * torch.stack([(-1) * sign_fc3_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.conv1.bias.grad = regu * net0.conv1.bias.data.clone() + penal * torch.stack([(-1) * sign_conv1_b_0_N[k] for k in range(10)],1).sum(axis=1)
        net0.conv2.bias.grad = regu * net0.conv2.bias.data.clone() + penal * torch.stack([(-1) * sign_conv2_b_0_N[k] for k in range(10)],1).sum(axis=1)
        optimizer0.step()
    ...
    pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_LR100%d.csv'%pr_c)
    pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_LR100%d.csv'%pr_c)
    pd.DataFrame(dist).to_csv('/home/hexchao/Simulation_CIFAR10/Dist_LR100%d.csv'%pr_c)
    pr_c += 1
