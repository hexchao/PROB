...
def DistinguishHuber(a,b):
    ...
    diff = a_line-b_line
    H_diff = penal * diff
    H_diff[diff.abs()>L_Huber] = diff[diff.abs()>L_Huber].sign() * penal
...
for pr_b in range(4):
    ...
    for epoch in range(iteration):
        ...
        for local_epoch in range(lc):
            ...
            for k in range(Node):
                net[k].fc1.weight.grad += DistinguishHuber(net[k].fc1.weight.data.clone(),net0.fc1.weight.data.clone())
                net[k].fc2.weight.grad += DistinguishHuber(net[k].fc2.weight.data.clone(),net0.fc2.weight.data.clone())
                net[k].fc3.weight.grad += DistinguishHuber(net[k].fc3.weight.data.clone(),net0.fc3.weight.data.clone())
                net[k].conv1.weight.grad += DistinguishHuber(net[k].conv1.weight.data.clone(),net0.conv1.weight.data.clone())
                net[k].conv2.weight.grad += DistinguishHuber(net[k].conv2.weight.data.clone(),net0.conv2.weight.data.clone())
                net[k].fc1.bias.grad += DistinguishHuber(net[k].fc1.bias.data.clone(),net0.fc1.bias.data.clone())
                net[k].fc2.bias.grad += DistinguishHuber(net[k].fc2.bias.data.clone(),net0.fc2.bias.data.clone())
                net[k].fc3.bias.grad += DistinguishHuber(net[k].fc3.bias.data.clone(),net0.fc3.bias.data.clone())
                net[k].conv1.bias.grad += DistinguishHuber(net[k].conv1.bias.data.clone(),net0.conv1.bias.data.clone())
                net[k].conv2.bias.grad += DistinguishHuber(net[k].conv2.bias.data.clone(),net0.conv2.bias.data.clone())
        ...
        if epoch==0:#给个初始值，不然GetSign_B和GetSub_B跑不了
            for k in range(Node):
                hb_fc1_w_0_N[k] = 0 * GetSign(net[k].fc1.weight.data.clone(),net0.fc1.weight.data.clone())
                hb_fc2_w_0_N[k] = 0 * GetSign(net[k].fc2.weight.data.clone(),net0.fc2.weight.data.clone())
                hb_fc3_w_0_N[k] = 0 * GetSign(net[k].fc3.weight.data.clone(),net0.fc3.weight.data.clone())
                hb_conv1_w_0_N[k] = 0 * GetSign(net[k].conv1.weight.data.clone(),net0.conv1.weight.data.clone())
                hb_conv2_w_0_N[k] = 0 * GetSign(net[k].conv2.weight.data.clone(),net0.conv2.weight.data.clone())
                hb_fc1_b_0_N[k] = 0 * GetSign(net[k].fc1.bias.data.clone(),net0.fc1.bias.data.clone())
                hb_fc2_b_0_N[k] = 0 * GetSign(net[k].fc2.bias.data.clone(),net0.fc2.bias.data.clone())
                hb_fc3_b_0_N[k] = 0 * GetSign(net[k].fc3.bias.data.clone(),net0.fc3.bias.data.clone())
                hb_conv1_b_0_N[k] = 0 * GetSign(net[k].conv1.bias.data.clone(),net0.conv1.bias.data.clone())
                hb_conv2_b_0_N[k] = 0 * GetSign(net[k].conv2.bias.data.clone(),net0.conv2.bias.data.clone())
        ...
        for k in range(Node):
            ...
            hb_fc1_w_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc1_w_0_N[k] for k in range(10)],2).sum(axis=2),net0.fc1.weight.data.clone())
            hb_fc2_w_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc2_w_0_N[k] for k in range(10)],2).sum(axis=2),net0.fc2.weight.data.clone())
            hb_fc3_w_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc3_w_0_N[k] for k in range(10)],2).sum(axis=2),net0.fc3.weight.data.clone())
            hb_conv1_w_0_B[k] = DistinguishHuber_B(torch.stack([hb_conv1_w_0_N[k] for k in range(10)],4).sum(axis=4),net0.conv1.weight.data.clone())
            hb_conv2_w_0_B[k] = DistinguishHuber_B(torch.stack([hb_conv2_w_0_N[k] for k in range(10)],4).sum(axis=4),net0.conv2.weight.data.clone())
            hb_fc1_b_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc1_b_0_N[k] for k in range(10)],1).sum(axis=1),net0.fc1.bias.data.clone())
            hb_fc2_b_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc2_b_0_N[k] for k in range(10)],1).sum(axis=1),net0.fc2.bias.data.clone())
            hb_fc3_b_0_B[k] = DistinguishHuber_B(torch.stack([hb_fc3_b_0_N[k] for k in range(10)],1).sum(axis=1),net0.fc3.bias.data.clone())
            hb_conv1_b_0_B[k] = DistinguishHuber_B(torch.stack([hb_conv1_b_0_N[k] for k in range(10)],1).sum(axis=1),net0.conv1.bias.data.clone())
            hb_conv2_b_0_B[k] = DistinguishHuber_B(torch.stack([hb_conv2_b_0_N[k] for k in range(10)],1).sum(axis=1),net0.conv2.bias.data.clone())
    ...
    pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_HH240%d.csv'%pr_c)
    pd.DataFrame(loss_list).to_csv('/home/hexchao/Simulation_CIFAR10/Loss_HH240%d.csv'%pr_c)
    pd.DataFrame(dist).to_csv('/home/hexchao/Simulation_CIFAR10/Dist_HH240%d.csv'%pr_c)
    pr_c += 1
