...
def DistinguishHuber_B(a,b):
    ...
    H_diff = penal * diff
    H_diff[diff.abs()>L_Huber] = diff[diff.abs()>L_Huber].sign()
    H_diff[diff.abs()<=L_Huber] = diff[diff.abs()<=L_Huber] * beta
    #H_diff = [penal*diff[i] if diff[i]>L_Huber else (1/L_Huber)*beta*diff[i] for i in range(len(diff))]
    H_diff = H_diff.view(a_size)
    return H_diff
...
for pr_b in range(4):
    ...
    for k in range(Node):
        ...
        netB.append(Net().to(device))
    for k in range(Node):
        net[k].load_state_dict(net0.state_dict())
        netB[k].load_state_dict(net0.state_dict())
    ...
    for k in range(Node):
        ...
        optimizerB.append(optim.SGD(netB[k].parameters(),lr = stepsize,momentum=mmm))
    ...
    outputsB = [[] for k in range(Node)]
    lossB = [[] for k in range(Node)]
    ...
    for epoch in range(iteration):
        ...
        for local_epoch in range(lc):
            ...
            for k in range(Node):
                ...
                loss[k].backward()
            for k in range(Node):
                optimizerB[k].zero_grad()
                outputsB[k] = netB[k](inputsB[k])
                lossB[k] = criterion(outputsB[k], labelsB[k])
                lossB[k].backward()
            ...
            for k in range(Node):
                optimizer[k].step()
            for k in range(Node):
                netB[k].fc1.weight.grad += beta * (netB[k].fc1.weight.data.clone() -net0.fc1.weight.data.clone())
                netB[k].fc2.weight.grad += beta * (netB[k].fc2.weight.data.clone() - net0.fc2.weight.data.clone())
                netB[k].fc3.weight.grad += beta * (netB[k].fc3.weight.data.clone() - net0.fc3.weight.data.clone())
                netB[k].conv1.weight.grad += beta * (netB[k].conv1.weight.data.clone() - net0.conv1.weight.data.clone())
                netB[k].conv2.weight.grad += beta * (netB[k].conv2.weight.data.clone() - net0.conv2.weight.data.clone())
                netB[k].fc1.bias.grad += beta * (netB[k].fc1.bias.data.clone() - net0.fc1.bias.data.clone())
                netB[k].fc2.bias.grad += beta * (netB[k].fc2.bias.data.clone() - net0.fc2.bias.data.clone())
                netB[k].fc3.bias.grad += beta * (netB[k].fc3.bias.data.clone() - net0.fc3.bias.data.clone())
                netB[k].conv1.bias.grad += beta * (netB[k].conv1.bias.data.clone() - net0.conv1.bias.data.clone())
                netB[k].conv2.bias.grad += beta * (netB[k].conv2.bias.data.clone() - net0.conv2.bias.data.clone())
            for k in range(Node):
                optimizerB[k].step()
        ...
        for k in range(Node):
            ...
            hb_fc1_w_0_B[k] = DistinguishHuber(netB[k].fc1.weight.data.clone(),net0.fc1.weight.data.clone())
            hb_fc2_w_0_B[k] = DistinguishHuber(netB[k].fc2.weight.data.clone(),net0.fc2.weight.data.clone())
            hb_fc3_w_0_B[k] = DistinguishHuber(netB[k].fc3.weight.data.clone(),net0.fc3.weight.data.clone())
            hb_conv1_w_0_B[k] = DistinguishHuber(netB[k].conv1.weight.data.clone(),net0.conv1.weight.data.clone())
            hb_conv2_w_0_B[k] = DistinguishHuber(netB[k].conv2.weight.data.clone(),net0.conv2.weight.data.clone())
            hb_fc1_b_0_B[k] = DistinguishHuber(netB[k].fc1.bias.data.clone(),net0.fc1.bias.data.clone())
            hb_fc2_b_0_B[k] = DistinguishHuber(netB[k].fc2.bias.data.clone(),net0.fc2.bias.data.clone())
            hb_fc3_b_0_B[k] = DistinguishHuber(netB[k].fc3.bias.data.clone(),net0.fc3.bias.data.clone())
            hb_conv1_b_0_B[k] = DistinguishHuber(netB[k].conv1.bias.data.clone(),net0.conv1.bias.data.clone())
            hb_conv2_b_0_B[k] = DistinguishHuber(netB[k].conv2.bias.data.clone(),net0.conv2.bias.data.clone())
    ...
    pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_PROBN251%d.csv'%pr_c)
    pr_c += 1
