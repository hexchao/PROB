...
Uk_g_fc1_w = [[] for k in range(Node)]
Uk_g_fc2_w = [[] for k in range(Node)]
Uk_g_fc3_w = [[] for k in range(Node)]
Uk_g_conv1_w = [[] for k in range(Node)]
Uk_g_conv2_w = [[] for k in range(Node)]
Uk_g_fc1_b = [[] for k in range(Node)]
Uk_g_fc2_b = [[] for k in range(Node)]
Uk_g_fc3_b = [[] for k in range(Node)]
Uk_g_conv1_b = [[] for k in range(Node)]
Uk_g_conv2_b = [[] for k in range(Node)]
...
for epoch in range(iteration):
    ...
    netV0.fc1.weight.grad = delta*(netV0.fc1.weight.data.clone()-netU0.fc1.weight.data.clone()) + delta*netU0.fc1.weight.data.clone() + torch.stack([hb_fc1_w_0_N[k] for k in range(10)],2).sum(axis=2)
    netV0.fc2.weight.grad = delta*(netV0.fc2.weight.data.clone()-netU0.fc2.weight.data.clone()) + delta*netU0.fc2.weight.data.clone() + torch.stack([hb_fc2_w_0_N[k] for k in range(10)],2).sum(axis=2)
    netV0.fc3.weight.grad = delta*(netV0.fc3.weight.data.clone()-netU0.fc3.weight.data.clone()) + delta*netU0.fc3.weight.data.clone() + torch.stack([hb_fc3_w_0_N[k] for k in range(10)],2).sum(axis=2)
    netV0.conv1.weight.grad = delta*(netV0.conv1.weight.data.clone()-netU0.conv1.weight.data.clone()) + delta*netU0.conv1.weight.data.clone() + torch.stack([hb_conv1_w_0_N[k] for k in range(10)],4).sum(axis=4)
    netV0.conv2.weight.grad = delta*(netV0.conv2.weight.data.clone()-netU0.conv2.weight.data.clone()) + delta*netU0.conv2.weight.data.clone() + torch.stack([hb_conv2_w_0_N[k] for k in range(10)],4).sum(axis=4)
    netV0.fc1.bias.grad = delta*(netV0.fc1.bias.data.clone()-netU0.fc1.bias.data.clone()) + delta*netU0.fc1.bias.data.clone() + torch.stack([hb_fc1_b_0_N[k] for k in range(10)],1).sum(axis=1)
    netV0.fc2.bias.grad = delta*(netV0.fc2.bias.data.clone()-netU0.fc2.bias.data.clone()) + delta*netU0.fc2.bias.data.clone() + torch.stack([hb_fc2_b_0_N[k] for k in range(10)],1).sum(axis=1)
    netV0.fc3.bias.grad = delta*(netV0.fc3.bias.data.clone()-netU0.fc3.bias.data.clone()) + delta*netU0.fc3.bias.data.clone() + torch.stack([hb_fc3_b_0_N[k] for k in range(10)],1).sum(axis=1)
    netV0.conv1.bias.grad = delta*(netV0.conv1.bias.data.clone()-netU0.conv1.bias.data.clone()) + delta*netU0.conv1.bias.data.clone() + torch.stack([hb_conv1_b_0_N[k] for k in range(10)],1).sum(axis=1)
    netV0.conv2.bias.grad = delta*(netV0.conv2.bias.data.clone()-netU0.conv2.bias.data.clone()) + delta*netU0.conv2.bias.data.clone() + torch.stack([hb_conv2_b_0_N[k] for k in range(10)],1).sum(axis=1)
    optimizerV0.step()
    ...
    for local_epoch in range(lc):
        ...
        for k in range(Node):
            Uk_g_fc1_w[k] = netUk[k].fc1.weight.grad.clone()
            Uk_g_fc2_w[k] = netUk[k].fc2.weight.grad.clone()
            Uk_g_fc3_w[k] = netUk[k].fc3.weight.grad.clone()
            Uk_g_conv1_w[k] = netUk[k].conv1.weight.grad.clone()
            Uk_g_conv2_w[k] = netUk[k].conv2.weight.grad.clone()
            Uk_g_fc1_b[k] = netUk[k].fc1.bias.grad.clone()
            Uk_g_fc2_b[k] = netUk[k].fc2.bias.grad.clone()
            Uk_g_fc3_b[k] = netUk[k].fc3.bias.grad.clone()
            Uk_g_conv1_b[k] = netUk[k].conv1.bias.grad.clone()
            Uk_g_conv2_b[k] = netUk[k].conv2.bias.grad.clone()
        ...
        for k in range(Node):
            netVk[k].fc1.weight.grad = delta*(netVk[k].fc1.weight.data.clone()-netUk[k].fc1.weight.data.clone()) + Uk_g_fc1_w[k] - Lambda * PenalHuber(netW0.fc1.weight.data.clone(),netWk[k].fc1.weight.data.clone())
            netVk[k].fc2.weight.grad = delta*(netVk[k].fc2.weight.data.clone()-netUk[k].fc2.weight.data.clone()) + Uk_g_fc2_w[k] - Lambda * PenalHuber(netW0.fc2.weight.data.clone(),netWk[k].fc2.weight.data.clone())
            netVk[k].fc3.weight.grad = delta*(netVk[k].fc3.weight.data.clone()-netUk[k].fc3.weight.data.clone()) + Uk_g_fc3_w[k] - Lambda * PenalHuber(netW0.fc3.weight.data.clone(),netWk[k].fc3.weight.data.clone())
            netVk[k].conv1.weight.grad = delta*(netVk[k].conv1.weight.data.clone()-netUk[k].conv1.weight.data.clone()) + Uk_g_conv1_w[k] - Lambda * PenalHuber(netW0.conv1.weight.data.clone(),netWk[k].conv1.weight.data.clone())
            netVk[k].conv2.weight.grad = delta*(netVk[k].conv2.weight.data.clone()-netUk[k].conv2.weight.data.clone()) + Uk_g_conv2_w[k] - Lambda * PenalHuber(netW0.conv2.weight.data.clone(),netWk[k].conv2.weight.data.clone())
            netVk[k].fc1.bias.grad = delta*(netVk[k].fc1.bias.data.clone()-netUk[k].fc1.bias.data.clone()) + Uk_g_fc1_b[k] - Lambda * PenalHuber(netW0.fc1.bias.data.clone(),netWk[k].fc1.bias.data.clone())
            netVk[k].fc2.bias.grad = delta*(netVk[k].fc2.bias.data.clone()-netUk[k].fc2.bias.data.clone()) + Uk_g_fc2_b[k] - Lambda * PenalHuber(netW0.fc2.bias.data.clone(),netWk[k].fc2.bias.data.clone())
            netVk[k].fc3.bias.grad = delta*(netVk[k].fc3.bias.data.clone()-netUk[k].fc3.bias.data.clone()) + Uk_g_fc3_b[k] - Lambda * PenalHuber(netW0.fc3.bias.data.clone(),netWk[k].fc3.bias.data.clone())
            netVk[k].conv1.bias.grad = delta*(netVk[k].conv1.bias.data.clone()-netUk[k].conv1.bias.data.clone()) + Uk_g_conv1_b[k] - Lambda * PenalHuber(netW0.conv1.bias.data.clone(),netWk[k].conv1.bias.data.clone())
            netVk[k].conv2.bias.grad = delta*(netVk[k].conv2.bias.data.clone()-netUk[k].conv2.bias.data.clone()) + Uk_g_conv2_b[k] - Lambda * PenalHuber(netW0.conv2.bias.data.clone(),netWk[k].conv2.bias.data.clone())
...
pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_FRPGN101%d.csv'%pr_c)
pr_c += 1
