...
for prB in range(4):
    ...
    for epoch in range(iteration):
        ...
        for k in range(Node):
            hb_fc1_w_0_B[k] = Lambda * PenalHuberB(netW0.fc1.weight.data.clone(),torch.stack([hb_fc1_w_0_N[k] for k in range(10)],2).sum(axis=2))
            hb_fc2_w_0_B[k] = Lambda * PenalHuberB(netW0.fc2.weight.data.clone(),torch.stack([hb_fc2_w_0_N[k] for k in range(10)],2).sum(axis=2))
            hb_fc3_w_0_B[k] = Lambda * PenalHuberB(netW0.fc3.weight.data.clone(),torch.stack([hb_fc3_w_0_N[k] for k in range(10)],2).sum(axis=2))
            hb_conv1_w_0_B[k] = Lambda * PenalHuberB(netW0.conv1.weight.data.clone(),torch.stack([hb_conv1_w_0_N[k] for k in range(10)],4).sum(axis=4))
            hb_conv2_w_0_B[k] = Lambda * PenalHuberB(netW0.conv2.weight.data.clone(),torch.stack([hb_conv2_w_0_N[k] for k in range(10)],4).sum(axis=4))
            hb_fc1_b_0_B[k] = Lambda * PenalHuberB(netW0.fc1.bias.data.clone(),torch.stack([hb_fc1_b_0_N[k] for k in range(10)],1).sum(axis=1))
            hb_fc2_b_0_B[k] = Lambda * PenalHuberB(netW0.fc2.bias.data.clone(),torch.stack([hb_fc2_b_0_N[k] for k in range(10)],1).sum(axis=1))
            hb_fc3_b_0_B[k] = Lambda * PenalHuberB(netW0.fc3.bias.data.clone(),torch.stack([hb_fc3_b_0_N[k] for k in range(10)],1).sum(axis=1))
            hb_conv1_b_0_B[k] = Lambda * PenalHuberB(netW0.conv1.bias.data.clone(),torch.stack([hb_conv1_b_0_N[k] for k in range(10)],1).sum(axis=1))
            hb_conv2_b_0_B[k] = Lambda * PenalHuberB(netW0.conv2.bias.data.clone(),torch.stack([hb_conv2_b_0_N[k] for k in range(10)],1).sum(axis=1))
    ...
    pd.DataFrame(accuracy_list).to_csv('/home/hexchao/Simulation_CIFAR10/Acc_FRPGN241%d.csv'%pr_c)
    pr_c += 1
